function button_generate_click(_arg) {
	...
	if (_arg == "batch") {
		...
		$('#interactiveButton').replaceWith('<button class="ui-state-default ui-corner-all big-button" id="interactiveButton"'
											+ 'onclick="button_generate_click(\'batch\')">Adjust layout</button>');
	}
	else {
		populateTable();
		$('#divSettings').replaceWith('<button class="ui-state-default ui-corner-all big-button" id="interactiveButton"'
									+ 'onclick="button_generate_click(\'manual\')">Adjust layout</button>');
	}
}
